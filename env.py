...
class FireFighter(dm_env.Environment):
  ...
  def burn_vertices(self):
    ...
    """
    burnable = np.logical_and(np.any(self.adj_mat[self.burned], axis=0), 
  ...
  def observation_spec(self):
    ...
    return (specs.Array(shape=self.adj_mat.shape, dtype=np.bool,
                        name="adjacency_matrix"),
            specs.Array(shape=(self.adj_mat.shape[0],), dtype=np.bool,
                        name="adjacency_matrix"),
            specs.Array(shape=(self.adj_mat.shape[0],), dtype=np.bool,
                        name="adjacency_matrix"),
    )
  ...
  def action_spec(self):
    ...
    return specs.Array(shape=(self.adj_mat.shape[0],), dtype=np.bool, 
                       name="defend_vertices")
